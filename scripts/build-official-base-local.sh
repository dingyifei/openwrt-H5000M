...
forbidden_packages='h5000m-fancontrol|luci-app-h5000m-fancontrol[^[:space:]]*|luci-app-h5000m-netmode[^[:space:]]*|luci-app-mt5700m[^[:space:]]*|travelmate|luci-app-travelmate[^[:space:]]*|mwan3|luci-app-mwan3[^[:space:]]*|tailscale|luci-app-tailscale[^[:space:]]*|openconnect|luci-proto-openconnect[^[:space:]]*|vpnc-scripts|lpac|luci-app-epm[^[:space:]]*|dnsmasq-full|kmod-nft-socket|kmod-nft-tproxy|luci-app-passwall[^[:space:]]*|luci-app-homeproxy[^[:space:]]*|luci-app-mosdns[^[:space:]]*|nikki[^[:space:]]*|luci-app-nikki[^[:space:]]*|mihomo[^[:space:]]*|xray-core[^[:space:]]*|xray-plugin[^[:space:]]*|sing-box[^[:space:]]*|hysteria[^[:space:]]*|tuic-client[^[:space:]]*|naiveproxy[^[:space:]]*|qmodem[^[:space:]]*|ubus-at-daemon[^[:space:]]*|sms-tool_q[^[:space:]]*|at-webserver[^[:space:]]*'
